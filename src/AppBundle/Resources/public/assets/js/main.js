...
(function($) {
	...
	$(function() {
		...
		skel.on('+narrower -narrower', function() {
			$.prioritize('.important\\28 narrower\\29', skel
					.breakpoint('narrower').active);
		});
		...
		$(
				'<div id="navButton">'
						+ '<a href="#navPanel" class="toggle"></a>' + '</div>')
				.appendTo($body);

		// Navigation Panel.
		$(
				'<div id="navPanel">' + '<nav>' + $('#nav').navList()
						+ '</nav>' + '</div>').appendTo($body).panel({
			delay : 500,
			hideOnClick : true,
			hideOnSwipe : true,
			resetScroll : true,
		...
		if (skel.vars.os == 'wp' && skel.vars.osVersion < 10)
			$('#navButton, #navPanel, body').css('transition', 'none');
		...
		if (!skel.vars.mobile && $header.hasClass('alt') && $banner.length > 0) {
			...
			$('#header ul li a').each( function( i, item )
			{
				item=$(item);
				item
			});
